...
class AGC {
  ...
  public function get_itunes_top_songs() {
    ...
    if ( file_exists( $cache_file ) ) {
      ...
      $count = get_option( 'itunes_count', 12 ) ;
      ...
      if ( $curl['info']['http_code'] == 200 ) {
        ...
        if ( isset( $xml['entry'] ) ) {
          foreach ( $xml['entry'] as $item ) {
           // print_r($item);
            //$data['duration'] =  $this->secont_to_minute($item['link'][1]['duration']);
          ...
          create_cache_json( $cache_file, $items );
        }
      }
    }
  ...
  public function get_itunes_new_releases() {
    ...
    if ( file_exists( $cache_file ) ) {
      ...
      $count = get_option( 'itunes_count', 6 );
          ...
          if ( $youtube__curl['info']['http_code'] == 200 ) {
            ...
            if ( isset( $youtube__array['items'] ) ) {
              foreach ( $youtube__array['items'] as $item ) {
                ...
                $data['title'] = $item['snippet']['title'];
                $data['channel'] = $item['snippet']['channelTitle'];
                $data['image'] = $item['snippet']['thumbnails']['default']['url'];
                // $data['image'] = $item['snippet']['thumbnails']['high']['url'];
                // $data['image'] = $item['snippet']['thumbnails']['medium']['url'];
                // http://i.ytimg.com/vi/dPI-mRFEIH0/default.jpg
                $items[] = $data;
              } 
              // create_cache_sqlite( $items, $key, 604800 ); //1 week
              // unset( $item, $data );
            }
          }
        }
        ...
        if ( $soundcloud__curl['info']['http_code'] == 200 ) {
          ...
          if ( is_array( $soundcloud__array ) ) {
            foreach ( $soundcloud__array as $item ) {              
              ...
              $data['image'] = ( isset( $item['artwork_url'] ) ) ? $item['artwork_url'] : 'http://1.bp.blogspot.com/-016PpiWrRh0/VH8FwkcV8lI/AAAAAAAAExg/vaZHFuzL5xU/s1600/default-logo.png';
              $items[] = $data;
            }
          }
        }
  ...
  public function get_download() {
    ...
    if ( file_exists( $cache_file ) ) {
        ...
        if ( $source == 'yt' ) {
          ...
          if ( $soundcloud__client_id ) {
            ...
            if ( $soundcloud__curl['info']['http_code'] ) {
              ...
              $data['user_permalink'] = $item['user']['permalink_url'];
      ...
      if ( isset( $data ) ) {
        create_cache_json( $cache_file, $data );
      }
    }
